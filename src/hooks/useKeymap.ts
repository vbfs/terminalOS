    ...
    const handleKeyDown = async (e: KeyboardEvent) => {
      ...
      if (matchesShortcut(e, 'CmdOrCtrl+D')) {
        ...
        if (activeTab && activePaneId) {
          ...
          sessionsStore.addSession({ id: sessionId, paneId: newPaneId, name: 'Session', cwd: cwd ?? '', status: 'running', aiProcess: null, tokens: 0, alertMessage: null, createdAt: Date.now() })
        }
        return
      }
