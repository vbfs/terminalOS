...
function createZdotdir() {
  const zdotdir = fs_1.default.mkdtempSync(
    path_1.default.join(os_1.default.tmpdir(), "aiterm-"),
  );
  const zprofile = [
  ...
  fs_1.default.writeFileSync(
    path_1.default.join(zdotdir, ".zprofile"),
    zprofile,
  );
  const zshrc = [
...
class WebFsWatcher {
  ...
  async readFile(filePath) {
    return promises_1.default.readFile(
  ...
  async mkdir(dirPath) {
    await promises_1.default.mkdir(path_1.default.resolve(dirPath), {
      recursive: true,
    });
  }
...
class WebVersionsManager {
  ...
  async load(filePath) {
    const vFile = path_1.default.join(
      this.getVersionsDir(),
...
function handleConnection(ws, versionsManager, pkgVersion) {
  ...
  ws.on("message", async (raw) => {
    ...
    try {
      ...
      if (id) respondError(id, err.message ?? "Internal error");
    }
  });
  ws.on("close", () => {
    ptyManager.killAll();
