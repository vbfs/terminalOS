...
class ProcessDetector {
  ...
  getCurrentAI(): AIProcess | null { return this.currentAI }
}
...
class WebPtyManager {
  ...
  create(opts: { cwd?: string; env?: Record<string, string> }): string {
    ...
    const session: PtySession = { id: sessionId, pty: ptyProcess, buffer: [], flushTimer: null, detector }
...
class WebFsWatcher {
  ...
  async readDir(dirPath: string): Promise<FsEntry[]> {
    ...
    await Promise.all(entries.map(async (entry) => {
      ...
      const stat = isDirectory ? null : await fsPromises.stat(entryPath).catch(() => null)
  ...
  async delete(targetPath: string): Promise<void> {
    await fsPromises.rm(path.resolve(targetPath), { recursive: true, force: true })
  }
  ...
  setWatchRoot(rootPath: string): void {
    ...
    const emit = (type: string, filePath: string) => this.send('fs:watch', [{ type, path: filePath }])
    this.watcher
      .on('add', (p) => emit('add', p))
...
interface FileVersion { id: string; version: number; timestamp: number; content: string }
interface VersionMeta { id: string; version: number; timestamp: number }
interface VersionsFile { filePath: string; versions: FileVersion[] }
...
class WebVersionsManager {
  ...
  private async load(filePath: string): Promise<FileVersion[]> {
    const vFile = path.join(this.getVersionsDir(), `${this.getKey(filePath)}.json`)
    try {
  ...
  async saveVersion(filePath: string, content: string): Promise<VersionMeta | null> {
    ...
    if (versions.length > 0 && versions[versions.length - 1].content === content) return null
    const nextVersion = versions.length > 0 ? versions[versions.length - 1].version + 1 : 1
    const now = Date.now()
    const newVersion: FileVersion = { id: new Date(now).toISOString(), version: nextVersion, timestamp: now, content }
...
async function openUrl(target: string): Promise<void> {
  const cmd = process.platform === 'darwin' ? `open "${target}"` :
               ...
               `xdg-open "${target}"`
  await execAsync(cmd).catch(() => { /* ignore */ })
}
...
function handleConnection(
  ...
  const respondError = (id: string, error: string) => {
    if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ id, error }))
  }
  ...
  ws.on('message', async (raw) => {
    ...
    try {
      switch (method) {
        // PTY
        case 'pty:create': {
          const sessionId = ptyManager.create(params as { cwd?: string; env?: Record<string, string> })
        ...
        case 'pty:write':
          ...
          break
        case 'pty:resize':
          ptyManager.resize(params.sessionId as string, params.cols as number, params.rows as number)
          break
        case 'pty:kill':
        ...
        case 'fs:writeFile':
          await fsWatcher.writeFile(params.path as string, params.content as string)
          if (id) respond(id, null)
          break
        case 'fs:writeBinaryFile':
          await fsWatcher.writeBinaryFile(params.filePath as string, params.data as string)
          if (id) respond(id, null)
          break
        case 'fs:mkdir':
        ...
        case 'fs:versions:save': {
          const meta = await versionsManager.saveVersion(params.filePath as string, params.content as string)
          if (id) respond(id, meta)
          break
        }
        case 'fs:versions:list': {
          const list = await versionsManager.listVersions(params.filePath as string)
          if (id) respond(id, list)
          break
        }
        case 'fs:versions:get': {
          const ver = await versionsManager.getVersion(params.filePath as string, params.versionId as string)
